...
geom_text_repel(data_otus_cos, mapping = aes(x,y, label=data_otus_cos$phyla), family = "Garamond", max.iter = 10000, size = 5) ## pour ajouter labels espèces
...
data_mean2 <- data_mean %>% 
  select(Pot_ID, OCS_flux, CO2_flux_aero, DarkLight) %>%
  group_by(Pot_ID, DarkLight) %>% summarize(mean_ocs_flux = mean(OCS_flux),
...
## CCA
data_log <- a_class %>% decostand("log")
data_cca <- cca(data_log ~ land_use + sand_content + ph + silt_content + cn_ratio + light_treatment + ocs_flux + co2_flux, data = soil_properties)
summary(data_cca)

data_log <- b_class %>% decostand("log")
data_cca <- cca(data_log ~ land_use + sand_content + ph + silt_content + cn_ratio + light_treatment + ocs_flux + co2_flux, data = soil_properties)
summary(data_cca)

data_log <- f_class %>% decostand("log")
data_cca <- cca(data_log ~ land_use + sand_content + ph + silt_content + cn_ratio + light_treatment + ocs_flux + co2_flux, data = soil_properties)
summary(data_cca)
...
graph_cca <- function(data, data_sp, data_gas) {
  ...
  data_plot <- data_plot + geom_point(data_otus, mapping = aes(x, y, size = relative_abun)) ## OTU POINTS
  ...
  data_otus$phyla <- abbreviate(data_otus$phyla, minlength = 3, use.classes = TRUE,
...
anosim(a_class[13:18], soil_properties$light_treatment, distance = "bray", permutations = 9999)
anosim(a_class[7:12], soil_properties$light_treatment, distance = "bray", permutations = 9999)
anosim(a_class[19:24], soil_properties$light_treatment, distance = "bray", permutations = 9999)
...
a_g <- a_genus %>% mutate(non_identified = Environmental + Unknown + Unclassified) %>% select(- Environmental, -Unknown, -Unclassified)
a_g <- a_g[19:24,]
sp_anova <- soil_properties %>% filter(soil == "TL")
adonis(a_g ~ light_treatment, data = sp_anova)

a_g <- a_genus %>% mutate(non_identified = Environmental + Unknown + Unclassified) %>% select(- Environmental, -Unknown, -Unclassified)
a_g <- a_g[7:12,]
sp_anova <- soil_properties %>% filter(soil == "LG")
anosim(a_g ~ light_treatment)
...
sp1 <- soil_properties %>% filter(soil == "DBZ")
alme <- lme(ocs_flux ~ light_treatment, random = ~1|chamber, data = sp1)
anova(alme)  # non significant
sp2 <- soil_properties %>% filter(soil == "LB")
blme <- lme(ocs_flux ~ light_treatment, random = ~1|chamber, data = sp2)
anova(blme) # non significant
sp3 <- soil_properties %>% filter(soil == "LG")
clme <- lme(ocs_flux ~ light_treatment, random = ~1|chamber, data = sp3)
anova(clme) # non significant
sp4 <- soil_properties %>% filter(soil == "TL")
dlme <- lme(ocs_flux ~ light_treatment, random = ~1|chamber, data = sp4)
anova(dlme) # 3 STARS

sp1 <- soil_properties %>% filter(soil == "DBZ")
alme <- lme(co2_flux ~ light_treatment, random = ~1|chamber, data = sp1)
anova(alme)  # non significant
sp2 <- soil_properties %>% filter(soil == "LB")
blme <- lme(co2_flux ~ light_treatment, random = ~1|chamber, data = sp2)
anova(blme) # non significant
sp3 <- soil_properties %>% filter(soil == "LG")
clme <- lme(co2_flux ~ light_treatment, random = ~1|chamber, data = sp3)
anova(clme) # non significant
sp4 <- soil_properties %>% filter(soil == "TL")
dlme <- lme(co2_flux ~ light_treatment, random = ~1|chamber, data = sp4)
anova(dlme) # 2 STARS
...
a_class_bar <- read.csv("algae/algae_class.csv",
                        ...
                        sep = ";",
                        fileEncoding="UTF-8-BOM")
a_class_bar <- a_class_bar %>% mutate(Unidentified = Unknown + Environmental + Unclassified) %>% select(-Unknown, -Environmental, -Unclassified)
...
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)
a_class_bar$soil <- factor(a_class_bar$soil,levels = c("DBZ4", "DBZ5", "DBZ6", "DBZ1", "DBZ2", "DBZ3",
                                                       "LB4", "LB5", "LB6", "LB1", "LB2", "LB3",
                                                       "LG4", "LG5", "LG6", "LG1", "LG2", "LG3",
                                                       "TL4", "TL5", "TL6", "TL1", "TL2", "TL3"))
x11(); a_class_bar %>% ggplot(aes(fill=otu, y=r_abundance2, x=soil)) + ## Lots of unknowns when soils are in the dark (probably unidentified cyanobacterias?). Lots of Chlorophyceae in LB dark. 
  geom_bar(position="stack", stat="identity") +              # Good amounts of Zygnemophyceae and Bacillariophyceae respectively in light LB and light DBZ.
  ...
  theme_ipsum_ps(axis_title_size = 13,
                 plot_title_face = "bold",
                 plot_title_size = 15) + scale_fill_manual(values = mycolors) 

ggsave("plots/4]Community barplots/a_class_community_reworked.png", plot = last_plot(), device = NULL, path = NULL,
       scale = 1, width = NA, height = NA, units = "cm",
       dpi = 300)
warnings()

adonis(a_class ~ light_treatment * soil, method = "bray", data = soil_properties, permutations = 9999)
adonis(b_class ~ light_treatment * soil, method = "bray", data = soil_properties, permutations = 9999)
adonis(f_class ~ light_treatment * soil, method = "bray", data = soil_properties, permutations = 9999)
...
b_class_bar <- read.csv("bacteria/bacteria_class.csv",
                        ...
                        sep = ";",
                        fileEncoding="UTF-8-BOM")
b_class_bar <- b_class_bar %>% mutate(Unidentified = Unknown + Environmental + Unclassified) %>% select(-Unknown, -Environmental, -Unclassified)
...
nb.cols <- b_class_bar %>% sapply(function(otu) length(unique(otu))) %>% 
  as.data.frame() %>% t() %>% as.data.frame() %>% select(otu) %>% as.matrix()
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)
b_class_bar$soil <- factor(b_class_bar$soil,levels = c("DBZ4", "DBZ5", "DBZ6", "DBZ1", "DBZ2", "DBZ3",
                                                       "LB4", "LB5", "LB6", "LB1", "LB2", "LB3",
                                                       "LG4", "LG5", "LG6", "LG1", "LG2", "LG3",
                                                       "TL4", "TL5", "TL6", "TL1", "TL2", "TL3"))
...
f_class_bar <- read.csv("fungi/fungi_class.csv",
                        ...
                        sep = "\t",
                        fileEncoding="UTF-8-BOM")
f_class_bar <- f_class_bar %>% mutate(Unidentified = Unknown + Environmental + Unclassified) %>% select(-Unknown, -Environmental, -Unclassified)
...
nb.cols <- f_class_bar %>% sapply(function(otu) length(unique(otu))) %>% 
  as.data.frame() %>% t() %>% as.data.frame() %>% select(otu) %>% as.matrix()
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)
f_class_bar$soil <- factor(f_class_bar$soil,levels = c("DBZ4", "DBZ5", "DBZ6", "DBZ1", "DBZ2", "DBZ3",
                                                       "LB4", "LB5", "LB6", "LB1", "LB2", "LB3",
                                                       "LG4", "LG5", "LG6", "LG1", "LG2", "LG3",
                                                       "TL4", "TL5", "TL6", "TL1", "TL2", "TL3"))
...
criterias
fitd <- gllvm(yd, Xd, family = "negative.binomial", num.lv = 5,
              formula = ~ ocs_flux + co2_flux, seed = 1234)
...
y <- a_class[,-(which(colSums(a_class) < 100))]
y$light_treatment <- soil_properties$light_treatment
y <- y %>% select(-Sphagnopsida, -Polytrichopsida, -Chlorokybophyceae)
y.filter <- y %>% select(Klebsormidiophyceae, Xanthophyceae, Dinophyceae,
                         Tetraphidopsida, Bacillariophyceae, Ulvophyceae,
                         Unknown, Phaeophyceae, Chlorophyceae, Zygnemophyceae,
                         Bryopsida, Takakiopsida, Pedinophyceae, light_treatment)
yl <- y.filter %>% filter(light_treatment == "light") %>% select(-light_treatment)
yl <- yl %>% select(-which(colSums(yl) < 100)) %>% as.matrix()
yd <- y.filter %>% filter(light_treatment == "dark") %>% select(-light_treatment) 
...
criterias
fitl <- gllvm(yl, Xl, family = "negative.binomial", num.lv = 4,
              formula = ~ ocs_flux + co2_flux, seed = 1234)
criterias <- NULL
for(i in 0:5){
  fiti <- gllvm(yd, Xd, family = "negative.binomial", num.lv = i, sd.errors = FALSE, 
...
criterias <- NULL
for(i in 0:5){
  fiti <- gllvm(y, Xd, family = "negative.binomial", num.lv = i, sd.errors = FALSE, 
                formula = ~ ocs_flux + co2_flux, seed = 1234)
  criterias[i + 1] <- summary(fiti)$AICc
  names(criterias)[i + 1] = i
}
...
otu.table.filter <- otu.table.filter %>% select(Klebsormidiophyceae,
                                                Xanthophyceae,
                                                Dinophyceae,
                                                Tetraphidopsida,
                                                Bacillariophyceae,
...
a_class_sr <- read.csv("algae/algae_class.csv",
                    dec = ",",
                    sep = ";",
                    fileEncoding="UTF-8-BOM",
                    row.names = 1)

a_class_bis <- read.csv("algae/algae_class.csv",
                       dec = ",",
                       sep = ";",
                       fileEncoding="UTF-8-BOM")

a_class_sr <- a_class_bis %>% select(-Environmental, -Unclassified, -Unknown) %>% mutate(a_sr=rowSums(.!=0)) %>% select(soil, a_sr)
b_class_sr <- b_class %>% select(-Environmental, -Unclassified, -Unknown) %>% mutate(b_sr=rowSums(.!=0)) %>% select(b_sr)
f_class_sr <- f_class %>% select(-Environmental, -Unclassified, -Unknown) %>% mutate(f_sr=rowSums(.!=0)) %>% select(f_sr)

sr_tab <- cbind(a_class_sr, b_class_sr, f_class_sr)
sr_tab$light <- soil_properties$light_treatment
sr_tab$sample <- soil_properties$soil
...
grouped_bars <- sr_tab %>% 
  ...
  summarise(sr = mean(a_sr),
            se = sd(a_sr / sqrt(length(a_sr)))) %>% 
  ggplot(sr_tab, 
         mapping = aes(x = sample, y = sr, fill = as.factor(light))) +
  geom_bar(stat="identity", position="dodge") +
  geom_hline(yintercept = 0, size = 1, colour="#333333") +
  scale_fill_manual(values = wes_palette("Chevalier1", n = 2)) +
  geom_errorbar(mapping = aes(ymin = sr - se, ymax = sr + se), 
                position = dodge,
                width = 0.2) +
  bbc_style()

x11(); plot(grouped_bars)


grouped_bars <- sr_tab %>% 
  group_by(sample, light) %>% 
  summarise(sr = mean(b_sr),
            se = sd(b_sr / sqrt(length(b_sr)))) %>% 
  ggplot(sr_tab, 
         mapping = aes(x = sample, y = sr, fill = as.factor(light))) +
  geom_bar(stat="identity", position="dodge") +
  geom_hline(yintercept = 0, size = 1, colour="#333333") +
...
grouped_bars <- sr_tab %>% 
  group_by(sample, light) %>% 
  summarise(sr = mean(f_sr),
            se = sd(f_sr / sqrt(length(f_sr)))) %>% 
  ggplot(sr_tab, 
         mapping = aes(x = sample, y = sr, fill = as.factor(light))) +
  geom_bar(stat="identity", position="dodge") +
  geom_hline(yintercept = 0, size = 1, colour="#333333") +
...
fungi_copies <- read.csv("18S_copies.csv",
                       dec = ",",
                       sep = ";",
                       fileEncoding="UTF-8-BOM")
soil_properties$fungi_copies <- fungi_copies$X18S_copies
soil_properties_bis$fungi_copies <- fungi_copies$X18S_copies
sp_bis_dark <- soil_properties_bis %>% filter(light_treatment == "Dark")
sp_bis_light <- soil_properties_bis %>% filter(light_treatment == "Light")
...
f_class <- read.csv("fungi/fungi_class.csv",
                    ...
                    row.names = 1)
y <- f_class[,-(which(colSums(f_class) < 100))] %>% as.matrix()
Xd <- sp_bis_dark %>% select(fungi_copies) %>% scale() %>% as.matrix()
Xl <- sp_bis_light %>% select(fungi_copies) %>% scale() %>% as.matrix()
criterias <- NULL
for(i in 0:5){
  fiti <- gllvm(y, Xl, family = "negative.binomial", num.lv = i, sd.errors = FALSE, 
                formula = ~ fungi_copies, seed = 1234)
  criterias[i + 1] <- summary(fiti)$AICc
  names(criterias)[i + 1] = i
}
criterias
...
ag_lightplot <- soil_properties %>% 
  ...
  geom_hline(yintercept = 0, size = 1, colour="#333333") +
  scale_fill_manual(values = wes_palette("Chevalier1", n = 2)) +
  geom_errorbar(mapping = aes(ymin = cos - se, ymax = cos + se), 
                position = dodge,
                width = 0.2) +
  bbc_style() +
  labs(fill = "Light treatment",
...
ag_co2plot <- soil_properties %>% 
  ...
  geom_hline(yintercept = 0, size = 1, colour="#333333") +
  scale_fill_manual(values = wes_palette("Chevalier1", n = 2)) +
  geom_errorbar(mapping = aes(ymin = co2 - se, ymax = co2 + se), 
                position = dodge,
                width = 0.2) +
  bbc_style()
x11(); print(ag_co2plot)
...
b_class_bar$soil <- factor(b_class_bar$soil,levels = c("DBZ4", "DBZ5", "DBZ6", "DBZ1", "DBZ2", "DBZ3",
                                                       ...
                                                       "LG4", "LG5", "LG6", "LG1", "LG2", "LG3",
                                                       "TL4", "TL5", "TL6", "TL1", "TL2", "TL3"))
x11(); b_class_bar %>% ggplot(aes(fill=otu, y=r_abundance2, x=soil)) + ## Lots of unknowns when soils are in the dark (probably unidentified cyanobacterias?). Lots of Chlorophyceae in LB dark. 
  geom_bar(position="stack", stat="identity") +              # Good amounts of Zygnemophyceae and Bacillariophyceae respectively in light LB and light DBZ.
  labs(subtitle = "Only bacterial classes with >0.5% of relative abundance in each soils are included",
       y = "Relative abundance (%)",
       x = "Soil sample",
...
transitivity(net.grph, type = c("global"), vids = NULL,    ## 0.45
             weights = NULL, isolates = c("zero"))         ##- 0.58
